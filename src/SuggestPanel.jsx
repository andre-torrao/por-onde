...
export default function SuggestPanel({ locationId, locationName, onClose }) {
  ...
  async function loadAll() {
    setLoading(true)
    const all = (() => { try { return JSON.parse(localStorage.getItem('pt_tracker_suggestions_v1') || '{}') } catch { return {} } })()
    setAllSuggs(all[locationId] || [])
    setLoading(false)
  }
  ...
  async function submit(e) {
    ...
    const ok = await addSuggestion({
      ...
      photo, authorId: user.id, authorUsername: user.id,
    })
    setBusy(false)
  ...
  const mine    = allSuggs.filter(s => s.author === (user?.username || user?.id) || s.author_id === user?.id)
  const pending = allSuggs.filter(s => s.status === 'pending' && s.author !== (user?.username || user?.id))
